...
```
## sohii 0.0.3 计划环境更新！
1. 解释器目前可以对变量和sohii函数进行注册（可覆盖），从而实现不同指令上下文之间的联系。    
2. sohii函数 关键字func 支持重载关键字 函数内是新指令，属于局部变量。        
   支持无参数、无输入、无输出、重载关键字、注册关键字模式 。      
   可以通过重载关键字以屏蔽原有关键字功能，从而限制嵌入python程序的sohii解释器控制的权限。
3. 支持sohii文件源代码文件，支持换行输入。       
4. 支持注释。
5. 实现封装进类，重构并且优化代码

## sohii 0.0.2 新特征！！！
1. if 指令支持立即数判定。 
2. 添加sohii_v和sohii指令。运行时获取版本信息。
3. 添加set指令，是obj_set的缩写版本。
4. 添加index_add和index_add_obj指令，实现相对位置跳转。
5. 添加int str obj float指令，单指令定义变量。
6. 添加input指令。支持从控制台获取字符串对象。
7. 添加copy关键字。用于对象的复制。
...
### 随机生成0-10之间的数字，直到生成10，累计生成的数。重复100次，求每次生成的值为。    
```python
a = " logging"
